...
func runBenchmark(name, driver, dsn string) {
	...
	_, err = db.Exec("CREATE TABLE `test` (`number` int(3) NOT NULL, `str` varchar(4) NOT NULL, PRIMARY KEY (`number`)) ENGINE=MyISAM DEFAULT CHARSET=utf8;")
	if err != nil {
		panic(err)
	}
...
func main() {
	//runBenchmark("Go-MySQL-Driver [run 1]", "mysql", "root:admin@tcp(192.168.1.241:3306)/test?charset=utf8")
	runBenchmark("mymysql godrv [run 1]", "mymysql", "tcp:192.168.1.22:3306*test/root/newding")
	//runBenchmark("Go-MySQL-Driver [run 2]", "mysql", "root:admin@tcp(192.168.1.241:3306)/test?charset=utf8")
	//runBenchmark("mymysql godrv [run 2]", "mymysql", "tcp:192.168.1.241:3306*test/root/admin")
}
